...
class TestEmrServerlessCreateApplicationOperator:
    @mock.patch("airflow.providers.amazon.aws.operators.emr.EmrServerlessHook.get_waiter")
    ...
    def test_execute_successfully_with_wait_for_completion(self, mock_conn, mock_waiter):
        mock_waiter.wait.side_effect = True
    ...
    def test_failed_create_application(self, mock_conn, mock_waiter):
        mock_waiter().wait = mock.Mock(
            side_effect=WaiterError(
                name="mock_waiter_error", reason="mock_reason", last_response="mock_response"
            )
        )
        mock_conn.create_application.return_value = {
            "applicationId": application_id,
    ...
    @mock.patch("airflow.providers.amazon.aws.operators.emr.EmrServerlessHook.get_waiter")
    @mock.patch("airflow.providers.amazon.aws.hooks.emr.EmrServerlessHook.conn")
    def test_failed_start_application(self, mock_conn, mock_waiter):
        mock_waiter().wait = mock.Mock(
            side_effect=WaiterError(
                name="mock_waiter_error", reason="mock_reason", last_response="mock_response"
            )
        ...
        mock_conn.get_application.call_count == 1
    ...
    @mock.patch("airflow.providers.amazon.aws.operators.emr.EmrServerlessHook.get_waiter")
    @mock.patch("airflow.providers.amazon.aws.hooks.emr.EmrServerlessHook.conn")
    def test_application_in_failure_state(self, mock_conn, mock_waiter):
        mock_waiter().wait = mock.Mock(
            side_effect=WaiterError(
                name="mock_waiter_error", reason="mock_reason", last_response="mock_response"
            )
...
class TestEmrServerlessStartJobOperator:
    ...
    @mock.patch("airflow.providers.amazon.aws.operators.emr.EmrServerlessHook.get_waiter")
    @mock.patch("airflow.providers.amazon.aws.hooks.emr.EmrServerlessHook.conn")
    def test_job_run_job_failed(self, mock_conn, mock_waiter):
        mock_waiter().wait = mock.Mock(
            side_effect=WaiterError(
                name="mock_waiter_error", reason="mock_reason", last_response="mock_response"
            )
        ...
        mock_conn.get_job_run.return_value = {"jobRun": {"state": "FAILED", "stateDetails": "Test Details"}}
    ...
    def test_job_run_app_not_started(self, mock_conn, mock_waiter):
        ...
        assert operator.wait_for_completion is True
        mock_conn.get_application.assert_called_once_with(applicationId=application_id)
        assert mock_waiter.call_count == 3
        assert id == job_run_id
        mock_conn.start_job_run.assert_called_once_with(
            clientToken=client_request_token,
            applicationId=application_id,
    ...
    @mock.patch("airflow.providers.amazon.aws.operators.emr.EmrServerlessHook.get_waiter")
    @mock.patch("airflow.providers.amazon.aws.hooks.emr.EmrServerlessHook.conn")
    def test_job_run_app_not_started_app_failed(self, mock_conn, mock_waiter):
        mock_waiter().wait = mock.Mock(
            side_effect=[
                True,
                WaiterError(name="mock_waiter_error", reason="mock_reason", last_response="mock_response"),
            ]
        )
        mock_conn.get_application.side_effect = [
    ...
    @mock.patch("airflow.providers.amazon.aws.operators.emr.EmrServerlessHook.get_waiter")
    @mock.patch("airflow.providers.amazon.aws.hooks.emr.EmrServerlessHook.conn")
    def test_start_job_run_fail_on_wait_for_completion(self, mock_conn, mock_waiter):
        mock_waiter().wait = mock.Mock(
            side_effect=[
                True,
                WaiterError(name="mock_waiter_error", reason="mock_reason", last_response="mock_response"),
    ...
    @mock.patch("airflow.providers.amazon.aws.operators.emr.EmrServerlessHook.get_waiter")
    @mock.patch("airflow.providers.amazon.aws.hooks.emr.EmrServerlessHook.conn")
    def test_start_job_default_name(self, mock_conn, mock_waiter):
        mock_waiter.wait.side_effect = True
        mock_conn.get_application.return_value = {"application": {"state": "STARTED"}}
        mock_conn.start_job_run.return_value = {
            "jobRunId": job_run_id,
    ...
    @mock.patch("airflow.providers.amazon.aws.operators.emr.EmrServerlessHook.get_waiter")
    @mock.patch("airflow.providers.amazon.aws.hooks.emr.EmrServerlessHook.conn")
    def test_start_job_custom_name(self, mock_conn, mock_waiter):
        mock_waiter.wait.side_effect = True
        mock_conn.get_application.return_value = {"application": {"state": "STARTED"}}
